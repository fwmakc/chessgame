import { GameState, Move, MoveResult, Position, Piece, VictoryCondition, Color, PieceType } from './types.js';
...
function hasNoPieces(state: GameState, color: Color): boolean {
...
export function makeMove(state: GameState, move: Move): MoveResult | null {
  ...
  let jumpCaptured: Piece | null = null;

  if (typeConfig) {
    for (const behavior of typeConfig.behaviors) {
      if (!behavior.jumpCapture) continue;
...
function isInsideBoard(state: GameState, pos: Position): boolean {
...
function hasNoTargetPiece(state: GameState, color: Color, targetType: PieceType): boolean {
  for (let row = 0; row < state.height; row++) {
    for (let col = 0; col < state.width; col++) {
      const piece = state.board[row][col];
      if (piece && piece.color === color && piece.type === targetType) return false;
    }
  }
  return true;
}

function hasNoLegalMoves(state: GameState, color: Color): boolean {
  return getAllLegalMoves(state, color).length === 0;
}

function allNonWallSquaresVisited(state: GameState): boolean {
  for (let row = 0; row < state.height; row++) {
    for (let col = 0; col < state.width; col++) {
...
export function isVictory(state: GameState): { winner: Color; reason: string } | null {
...
export function isGameOver(state: GameState): boolean {
  if (state.gameTimedOut) return true;
  if (isVictory(state)) return true;
  if (hasNoLegalMoves(state, state.turn)) return true;
  return false;
}
