export async function loadBoardConfig(url) {
    const res = await fetch(url);
    if (!res.ok)
        throw new Error(`Failed to load board config: ${res.status}`);
    return res.json();
}
export async function loadGameConfig(url) {
    const res = await fetch(url);
    if (!res.ok)
        throw new Error(`Failed to load game config: ${res.status}`);
    return res.json();
}
function getDefaultInitialPieces(height) {
...
export function createInitialState(config) {
    ...
    const squares = config?.squares ?? Array.from({ length: height }, (_, r) => Array.from({ length: width }, (_, c) => ((r + c) % 2 === 0 ? 'light' : 'dark')));
    const board = Array.from({ length: height }, () => Array(width).fill(null));
    const visitedSquares = Array.from({ length: height }, () => Array(width).fill(false));
    const pieceHistory = new Map();
    let nextId = 1;
    ...
    for (const p of initialPieces) {
        if (p.row >= 0 && p.row < height && p.col >= 0 && p.col < width && squares[p.row][p.col] !== 'wall') {
            const piece = { id: nextId++, type: p.type, color: p.color };
...
export function cloneState(state) {
    return {
        ...
        pieceHistory: new Map(Array.from(state.pieceHistory.entries()).map(([id, grid]) => [id, grid.map(row => [...row])])),
...
export function deserializeState(data) {
    return {
        ...
        pieceHistory: new Map(Object.entries(data.pieceHistory).map(([k, v]) => [Number(k), v])),
...
export function isInsideBoard(state, pos) {
    return pos.row >= 0 && pos.row < state.height && pos.col >= 0 && pos.col < state.width;
}
export function getPiece(state, pos) {
    if (!isInsideBoard(state, pos))
        return null;
    return state.board[pos.row][pos.col];
}
export function getSquareType(state, pos) {
    if (!isInsideBoard(state, pos))
        return null;
    return state.squares[pos.row][pos.col];
}
export function setPiece(state, pos, piece) {
    if (!isInsideBoard(state, pos))
        return;
...
export function stringToPos(s, height = 8) {
    const files = 'abcdefghijklmnopqrstuvwxyz';
    return {
