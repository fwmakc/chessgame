...
export async function loadPieceConfig(url) {
    const res = await fetch(url);
    if (!res.ok)
        throw new Error(`Failed to load piece config: ${res.status}`);
    const config = await res.json();
    globalConfig = config;
    return config;
}
export function setPieceConfig(config) {
    globalConfig = config;
}
export function getPieceConfig() {
    if (!globalConfig)
        throw new Error('Piece config not loaded');
    return globalConfig;
}
export function getConfigMoves(state, pos, pieceType, color) {
    ...
    for (const behavior of typeConfig.behaviors) {
        if (!checkConditions(state, pos, color, behavior.conditions))
            continue;
            ...
            if (behavior.slide) {
                if (behavior.jumpCapture) {
                    ...
                    while (isInsideBoard(state, { row: r, col: c })) {
                        ...
                        if (getSquareType(state, targetPos) === 'wall')
                            break;
                ...
                if (!isInsideBoard(state, targetPos))
                    continue;
                if (getSquareType(state, targetPos) === 'wall')
                    continue;
                if (behavior.conditions?.includes('clearPath')) {
                    if (!isPathClear(state, pos, targetPos))
                        continue;
                }
                if (behavior.jumpCapture) {
                    const midPos = { row: pos.row + dy / 2, col: pos.col + dx / 2 };
                    if (!isInsideBoard(state, midPos))
                        continue;
                    const targetPiece = getPiece(state, targetPos);
...
function checkConditions(state, pos, color, conditions) {
    if (!conditions)
        return true;
...
function isPathClear(state, from, to) {
    ...
    while (r !== to.row || c !== to.col) {
        const p = { row: r, col: c };
        if (!isInsideBoard(state, p))
            return false;
        if (getSquareType(state, p) === 'wall')
            return false;
        if (getPiece(state, p))
            return false;
        r += dr;
