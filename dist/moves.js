...
export function getPseudoLegalMoves(state, pos) {
...
function getEnPassantMoves(state, pos, color) {
    const moves = [];
    ...
    for (const dc of [-1, 1]) {
        ...
        if (isInsideBoard(state, capturePos) &&
...
function getCastlingMoves(state, pos, color, rules) {
    const moves = [];
    const enemyColor = color === 'white' ? 'black' : 'white';
    for (const rule of rules) {
        // Check king hasn't moved
        if (state.pieceHasMoved[pos.row][pos.col])
            continue;
        ...
        if (!isInsideBoard(state, kingTo))
            continue;
        // Check king not in check, doesn't pass through check, doesn't land in check
        if (isSquareAttacked(state, pos, enemyColor))
            continue;
        const midCol = pos.col + step;
        if (isSquareAttacked(state, { row: pos.row, col: midCol }, enemyColor))
            continue;
        if (isSquareAttacked(state, kingTo, enemyColor))
            continue;
        moves.push({ from: pos, to: kingTo });
    }
    return moves;
}
export function isSquareAttacked(state, pos, byColor) {
...
export function getLegalMoves(state, pos) {
