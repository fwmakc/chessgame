...
class ChessGame {
    constructor() {
        ...
        this.currentConfigUrl = GAME_CONFIGS[0];
        this.skins = new Map();
        this.currentSkinId = 'default';
        this.moveTimerInterval = null;
        this.moveTimerDeadline = 0;
        this.gameTimerInterval = null;
        this.gameTimerDeadline = 0;
    ...
    applySettingsToUI() {
        ...
        if (gameSelect)
            gameSelect.value = this.currentConfigUrl;
        const skinSelect = document.getElementById('skin-select');
        if (skinSelect)
            skinSelect.value = this.currentSkinId;
        ...
        if (aiColorSelect)
            aiColorSelect.value = this.playerColor;
        const flipCheckbox = document.getElementById('flip-board');
        if (flipCheckbox)
            flipCheckbox.checked = this.flipBoard;
    }
    applySkin(skinId) {
        ...
        let boardLink = document.getElementById('skin-board-css');
        if (!boardLink) {
    ...
    confirmRestart(onConfirm, onCancel) {
    ...
    startMoveTimer(remainingSeconds) {
        this.stopMoveTimer();
    ...
    updateMoveTimerDisplay(remaining) {
    ...
    startGameTimer(remainingSeconds) {
        this.stopGameTimer();
    ...
    updateGameTimerDisplay(remainingSeconds) {
    ...
    setupUI() {
        ...
        document.getElementById('flip-board').addEventListener('change', (e) => {
            this.flipBoard = e.target.checked;
            this.saveSettings();
            this.render();
        });
    ...
    `;
        ...
        modal.querySelector('.promotion-options').addEventListener('click', (e) => {
            const target = e.target.closest('.promotion-option');
            if (target) {
    ...
    onSquareClick(row, col) {
        if (this.aiThinking || this.promotionPending)
            return;
        if (this.mode === 'ai' && this.state.turn !== this.playerColor)
            return;
        if (isGameOver(this.state))
            return;
        const pos = { row, col };
    ...
    executeMove(move) {
        ...
        if (result.wasJumpCapture && piece) {
            ...
            const hasMultiCapture = typeConfig?.behaviors.some((b) => b.conditions?.includes('multiCapture')) ?? false;
            if (hasMultiCapture) {
    ...
    addMoveToHistory(result, moveTime) {
